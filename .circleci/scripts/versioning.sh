...
function bumpPackageVersion {
    if [ $(matchCommitMessagePattern "build|chore|ci|docs|fix|perf|refactor|style|test") != "0" ]; then
        ...
        echo "Commit message should start with a valid type, please see https://github.com/tinkeringprogrammer/angular-confirmation-capture/blob/main/CONTRIBUTING.md to learn more"
        npm version minor --no-git-tag-version
    fi
...
if [ $(matchCommitMessagePattern "fix|refactor|feat|breaking(\s*change)?") != "0" ]; then
    ...
    git add {.,projects/angular-confirmation-capture}/package.json
